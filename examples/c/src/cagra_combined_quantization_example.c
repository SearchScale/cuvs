...
typedef struct {
    ...
    float float32_recall, fp16_recall, quantized_recall;
    long float32_index_size, fp16_index_size, quantized_index_size;
} BenchmarkResults;
...
void evaluate_and_print_recall(uint32_t *float32_neighbors, uint32_t *fp16_neighbors, uint32_t *quantized_neighbors,
                              uint32_t **ground_truth, BenchmarkResults* results) {
    printf("\n=== RECALL EVALUATION ===\n");
    float total_float32_recall = 0.0f, total_fp16_recall = 0.0f, total_quantized_recall = 0.0f;
    ...
    for (int q = 0; q < N_QUERIES; ++q) {
        ...
        printf("Query %d - Float32: %.1f%%, FP16: %.1f%%, 8-bit: %.1f%%\n", 
               q, float32_recall, fp16_recall, quantized_recall);
...
uint32_t* run_quantized_cagra_workflow(cuvsResources_t res, BenchmarkResults* results) {
    printf("\n--- 8-bit Scalar Quantization Workflow ---\n");
    ...
    printf("8-bit quantized index build time: %.3f seconds\n", results->quant_build_time);
    ...
    cuvsCagraSearchParams_t quant_search_params = create_search_params();
    ...
    printf("8-bit quantized search time: %.3f seconds\n", results->quant_search_time);
    ...
    // Serialize the index to measure file size
    const char* quantized_index_file = "/tmp/cagra_8bit_index.bin";
    cuvsCagraSerialize(res, quantized_index_file, quant_index, true);
    results->quantized_index_size = get_file_size(quantized_index_file);
    printf("8-bit quantized index serialized to %s (size: %ld bytes, %.2f MB)\n", 
           quantized_index_file, results->quantized_index_size, results->quantized_index_size / (1024.0 * 1024.0));
...
void cagra_build_search_test(const char* dataset_type, void (*dataset_generator)(), BenchmarkResults* results) {
    ...
    uint32_t *float32_neighbors_h = run_float32_cagra_workflow(res, results);
    uint32_t **ground_truth = compute_ground_truth();
    
    uint32_t *fp16_neighbors_h = run_fp16_cagra_workflow(res, results);
    uint32_t *quantized_neighbors_h = run_quantized_cagra_workflow(res, results);
    
    evaluate_and_print_recall(float32_neighbors_h, fp16_neighbors_h, quantized_neighbors_h, ground_truth, results);

    free(quantized_neighbors_h);
    free(fp16_neighbors_h);
    free(float32_neighbors_h);
    for (int q = 0; q < N_QUERIES; ++q) {
        free(ground_truth[q]);
    }
    free(ground_truth);
...
void print_summary_table() {
    ...
    printf("┌───────────┬─────┬─────┬─────┬───────────┬───────────┬───────────┬───────────┬───────────┬───────────┬─────────┬─────────┬─────────┐\n");
    printf("│ Dataset   │  N  │ Dim │TopK │ F32 Build │ F32 Search│F16 Build  │F16 Search │ 8b Build  │ 8b Search │ F32 Rec │F16 Rec  │ 8b Rec  │\n");
    printf("│ Type      │ (k) │     │     │ (sec)     │ (sec)     │ (sec)     │ (sec)     │ (sec)     │ (sec)     │ (%%)     │ (%%)     │ (%%)     │\n");
    printf("├───────────┼─────┼─────┼─────┼───────────┼───────────┼───────────┼───────────┼───────────┼───────────┼─────────┼─────────┼─────────┤\n");
    printf("│ Random    │ %3d │%4d │ %3d │ %9.3f │ %9.3f │ %9.3f │ %9.3f │ %9.3f │ %9.3f │ %7.1f │ %7.1f │ %7.1f │\n",
           ...
           random_results.float32_recall, random_results.fp16_recall, random_results.quantized_recall);
    printf("│ Clustered │ %3d │%4d │ %3d │ %9.3f │ %9.3f │ %9.3f │ %9.3f │ %9.3f │ %9.3f │ %7.1f │ %7.1f │ %7.1f │\n",
           N_ROWS/1000, N_COLS, TOPK,
           ...
           clustered_results.float32_recall, clustered_results.fp16_recall, clustered_results.quantized_recall);
    printf("└───────────┴─────┴─────┴─────┴───────────┴───────────┴───────────┴───────────┴───────────┴───────────┴─────────┴─────────┴─────────┘\n");
    
    printf("\n=== SPEEDUP ANALYSIS ===\n");
    printf("┌───────────┬───────────┬───────────┬───────────┬───────────┬─────────────│─────────────┐\n");
    printf("│ Dataset   │F16 Build  │F16 Search │ 8b Build  │ 8b Search │ FP16 Rec    │ 8-bit Rec   │\n");
    printf("│ Type      │(x faster) │(x faster) │(x faster) │(x faster) │ Loss (%%)    │ Loss (%%)    │\n");
    printf("├───────────┼───────────┼───────────┼───────────┼───────────┼─────────────┼─────────────┤\n");
    printf("│ Random    │ %9.1fx │ %9.1fx │ %9.1fx │ %9.1fx │ %11.1f │ %11.1f │\n",
    ...
    printf("│ Clustered │ %9.1fx │ %9.1fx │ %9.1fx │ %9.1fx │ %11.1f │ %11.1f │\n",
    ...
    printf("┌───────────┬─────────────┬─────────────┬─────────────┬─────────────┬─────────────┬─────────────┐\n");
    printf("│ Dataset   │F32 Size (MB)│F16 Size (MB)│ 8b Size (MB)│F16/F32 Ratio│ 8b/F32 Ratio│Space Savings│\n");
    printf("│ Type      │             │             │             │             │             │  FP16 / 8b  │\n");
    printf("├───────────┼─────────────┼─────────────┼─────────────┼─────────────┼─────────────┼─────────────┤\n");
    printf("│ Random    │ %11.2f │ %11.2f │ %11.2f │ %11.3f │ %11.3f │ %5.1f%% /%5.1f%% │\n",
           random_results.float32_index_size / (1024.0 * 1024.0),
           random_results.fp16_index_size / (1024.0 * 1024.0),
           random_results.quantized_index_size / (1024.0 * 1024.0),
           (double)random_results.fp16_index_size / random_results.float32_index_size,
           (double)random_results.quantized_index_size / random_results.float32_index_size,
           (1.0 - (double)random_results.fp16_index_size / random_results.float32_index_size) * 100.0,
           (1.0 - (double)random_results.quantized_index_size / random_results.float32_index_size) * 100.0);
    printf("│ Clustered │ %11.2f │ %11.2f │ %11.2f │ %11.3f │ %11.3f │ %5.1f%% /%5.1f%% │\n",
           clustered_results.float32_index_size / (1024.0 * 1024.0),
           clustered_results.fp16_index_size / (1024.0 * 1024.0),
           clustered_results.quantized_index_size / (1024.0 * 1024.0),
           (double)clustered_results.fp16_index_size / clustered_results.float32_index_size,
           (double)clustered_results.quantized_index_size / clustered_results.float32_index_size,
           (1.0 - (double)clustered_results.fp16_index_size / clustered_results.float32_index_size) * 100.0,
           (1.0 - (double)clustered_results.quantized_index_size / clustered_results.float32_index_size) * 100.0);
    printf("└───────────┴─────────────┴─────────────┴─────────────┴─────────────┴─────────────┴─────────────┘\n");
}

int main() {
    printf("=== CAGRA COMBINED QUANTIZATION COMPARISON ===\n\n");
    printf("This example compares three CAGRA quantization approaches:\n");
    printf("1. Float32 (unquantized baseline)\n");
    printf("2. FP16 (half-precision floating point)\n");
    printf("3. 8-bit scalar quantization (int8)\n\n");
    printf("All approaches use the same dataset for fair comparison.\n");
    printf("Index sizes are measured by serializing to files.\n\n");
